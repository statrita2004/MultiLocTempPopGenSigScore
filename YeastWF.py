...
if data_choice == 0:
    ## K dataset ##
    hap_freq = [[0.0319, 0.1644, 0.1852, 0.1487, 0.011, 0.0254, 0.2716, 0.1619]]
    yeast_data = torch.tensor([[[0.00, 0.07692308, 0.34782609, 0.20689655]],
    [[0.50, 0.4579439, 0.1747573, 0.4519231]],
    [[1.00, 0.5079365, 0.1333333, 0.5073529]]], dtype=torch.float64)
elif data_choice == 1:
    ## S dataset ##
    hap_freq = [[0.1411, 0.5509, 0.0168, 0.0566, 0.12, 0.071, 3e-04, 0.0432]]
    yeast_data = torch.tensor([[[0.00,  0.20689655, 0.08943089, 0.70642202]],
...
WF = WF(loss_fn = neg_approx_llhd, n_parameter = n_param, population_size=2000,
        ...
        haplotype_freq = hap_freq[0], dominance_param = None)
...
WFU= WFU(neg_approx_llhd, population_size=2000, generation=210, generation_interval=[105, 210],
         haplotype_freq = hap_freq[0],
                 recomb_param=r[0], data_obs=yeast_data, n_sample=100, n_param=n_param, n_ensemble=10)
lpost = lambda x: WFU.llhd_grad(x) + WFU.logprior_grad(x)

## Compute LLS Estimate ###
##############################################
data_obs_tmp_numpy = yeast_data[:,:,:].detach().numpy().squeeze()
...
for ind_dim in range(data_obs_tmp_numpy.shape[1]-1):
    ...
    if p_t == p_0:
        ...
        estimate_lls[ind_dim] = (np.log((p_t * (1 - p_0)) / (p_0 * (1 - p_t)))) * (
                    2 / (105 * nonzero_index))
...
#                                      [0, 1, 0],
#                                      [0, 0, 1]], dtype=torch.float64), nmoves=5000, return_entire_chain=True, adapt=True)
# xxa = []
...
if data_choice == 0:
    #np.savez('Results/Yeast/Yeast_K',samples=xxa, samplesR = xx)
    xx = np.load('Results/Yeast/Yeast_K.npz')['samples']
elif data_choice == 1:
...
print('[',str(np.quantile(xx[:,2],0.05))+','+str(np.quantile(xx[:,2],0.95))+']')

if data_choice == 0:
    print('K-data: LLS estimate:', estimate_lls)
    print('K-data: Posterior Mean:', postmean)
elif data_choice == 1:
    print('S-data: LLS estimate:', estimate_lls)
    print('S-data: Posterior Mean:', postmean)
...
plt.ylim([ymin, ymax])
# plt.colorbar()
# plt.plot(xx[:,i], xx[:,j], 'k.', markersize=5, label='samples', alpha=0.01)
plt.plot(postmean[i], postmean[j], 'rx', markersize=30, label='posterior mean')
plt.plot(estimate_lls[i], estimate_lls[j], 'bx', markersize=30, label='LLS estimate')
#plt.plot(s[0][i], s[0][j], 'bx', markersize=30, label='true value')
plt.legend(fontsize=20)
plt.xlabel(r'$s_1$',fontsize=15)
plt.ylabel(r'$s_2$',fontsize=15)
if data_choice == 0:
    plt.savefig('Results/Yeast/Yeast_K_'+str(i)+str(j)+'.jpg')
elif data_choice == 1:
