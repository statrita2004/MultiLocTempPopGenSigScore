...
# ###### Choose different experimental setups for n_param=1 ########################
...
FDSU= FDSU(neg_approx_llhd, population_size=5000, generation=100, generation_interval=[10],
                 recomb_param=r[0], haplotype_freq=[0.4, 0.6],
         data_obs=data_obs, n_sample=100, n_param=1, n_ensemble= 10)
...
xx = MH(lpost, x0 = x0,
        sigma = torch.tensor(1e-1), nmoves=5000, return_entire_chain=True, adapt=False)
...
plt.legend()
plt.savefig('Results/FDS/MH_FDS_1.png')
plt.close()