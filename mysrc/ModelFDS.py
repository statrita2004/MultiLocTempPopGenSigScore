...
class ModelWrightFisherFDS():
    ...
    def forward_simulation(self, parameters, n_data=1):
        ...
        #W_ij = self.compute_fitness_matrix_L_loci(s=selection_coeff, h=self.dominance_param)
        ...
        vector_of_k_samples = self.wf_sim(s=selection_coeff, h=self.dominance_param,
    ...
    def compute_fitness_matrix_L_loci(self, s, h, allel_frequency):
        """
        ...
        """
        if self.L == 1:
            W = np.array([[1, 1 - allel_frequency[0] * s[0] * h[0]],
                          [1 - allel_frequency[0] * s[0] * h[0], 1 - allel_frequency[0] * s[0]]])
        else:
            w_l_tensor = []
            for l in range(self.L):
                # create single (2x2) w_l matrices
                w_l_array = np.array([[1, 1 - allel_frequency[l] * s[l] * h[l]],
                                      [1 - allel_frequency[l] * s[l] * h[l], 1 - allel_frequency[l] * s[l]]])
    ...
    def wf_sim(self, s, h,
        ...
        initial_allele_freq = self.calculate_one_allele_freq(initial_hapl_freq)
        for iter_simul in range(num_forward_simulations):
            #print('Number of simulation:', iter_simul)
            allele_freq_trajectory = []
            haplo_freq_trajectory = []
            allele_freq_one_gen = initial_allele_freq
            haplo_freq_trajectory.append(initial_hapl_freq)
            allele_freq_trajectory.append(allele_freq_one_gen)
            for iter_gen in range(last_gen):
                #print('Number of generation:', iter_gen)
                fitness_matrix = self.compute_fitness_matrix_L_loci(s=s, h=h, allel_frequency=allele_freq_one_gen)
                #print(fitness_matrix.shape)

                haplo_freq_one_gen = self.compute_haplo_freq_traj(w=fitness_matrix,
                                                                     x_initial=initial_hapl_freq,
                                                                     gen=1,
                                                                     population_size=population_size,
                                                                     rng=rng)[-1,:]
                haplo_freq_trajectory.append(haplo_freq_one_gen)
                allele_freq_one_gen = self.calculate_one_allele_freq(haplo_freq_one_gen)
                allele_freq_trajectory.append(allele_freq_one_gen)

            # haplo
            haplo_freq_trajectory = np.array(haplo_freq_trajectory)[timestep_generation,:]
            # allele
            allele_freq_trajectory = np.array(allele_freq_trajectory)[timestep_generation,:]

            # Add timesteps to allele frequency data
...
class FDSUniform():
    def __init__(self, neg_approx_llhd, population_size, generation, generation_interval,
        ...
        self.core = ModelWrightFisherFDS(neg_approx_llhd, n_param, population_size, generation, generation_interval,
...
class ModelWrightFisherFDShapfreq():
    ## Help file and description of the inputs ##
    def __init__(self, loss_fn, n_parameter, population_size, generation, generation_interval,
...
class FDSUniformDirichlet2():
    def __init__(self, neg_approx_llhd, population_size, generation, generation_interval,
        ...
        self.core = ModelWrightFisherFDShapfreq(neg_approx_llhd, n_param, population_size, generation, generation_interval,
