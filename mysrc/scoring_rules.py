...
class EnergyScore(ScoringRule):
    ...
    def __init__(self, beta=1, mean=False):
...
class ScaledEnergyScore(ScoringRule):
    """ Estimates the EnergyScore. Here, I assume the observations and simulations are lists of
    length respectively n_obs and n_sim. Then, for each fixed observation the n_sim simulations are used to estimate the
    scoring rule. Subsequently, the values are summed over each of the n_obs observations.
    ...
    def __init__(self, beta=1, mean=False):
        """default value is beta=1"""
        self.beta = beta
        self.logger = logging.getLogger("Scaled Energy Score")
    ...
    def estimate_energy_score_numpy(self, observations, simulations):
        ...
        ## Above computes Energy Score, Now we transform this to Scaled Energy Score
    ...
    def estimate_score_batch(self, forecast: TensorType["batch", "ensemble_size", "data_size"],
        ...
        first_term_y = torch.mean(diff_X_y, dim=1)
        second_term_y = torch.sum(diff_X_tildeX, dim=(1,2)) / (ensemble_size * (ensemble_size - 1))

        print(first_term_y, second_term_y)

        first_term_y = first_term_y[1:]
        second_term_y = second_term_y[1:]

        result = torch.sum(1 + (first_term_y / second_term_y) + torch.log(2 * second_term_y))
...
class KernelScore(ScoringRule):
    ...
    def __init__(self, kernel="gaussian", biased_estimator=False, torch=True, mean=False, **kernel_kwargs):
        """
        Parameters
        ----------
...
class VariogramScore(ScoringRule):
    ...
    def __init__(self, p: float = 1.0, variogram: TensorType["data_size", "data_size"] = None, mean=False,
                 max_batch_size=None):
        """default value is beta=1"""
        self.p = p
...
class SignatureKernel(ScoringRule):
    ...
    def __init__(self, dyadic_order = 2, keep_time = False, cumsum=False, static_kernel_name = None, rbf_sigma=1, omega = 1):
        self.logger = logging.getLogger("Signature Kernel Score")
        self.rbf_sigma = rbf_sigma
        self.dyadic_order = dyadic_order
        self.cumsum = cumsum
        self.keep_time = keep_time
        self.static_kernel_name = static_kernel_name
        self.omega = omega
    ...
    def estimate_score_batch(self, forecast: TensorType["batch", "ensemble_size", "data_size"],
        ...
        if self.cumsum:
            forecast[:,:,1:] = torch.cumsum(verification[:,:,1:], dim=0)
            verification[:,:,1:] = torch.cumsum(verification[:,:,1:], dim=0)

        if self.keep_time is False:
            forecast = forecast[:,:,1:]
            verification = verification[:,:,1:]

        # forecast = rearrange(forecast, 't b d -> d t b')
        # verification = rearrange(verification, 't b d -> d t b')

        forecast = rearrange(forecast, 't b d -> b t d')
        verification = rearrange(verification, 't b d -> b t d')

        if self.static_kernel_name == 'RBF':
            static_kernel = sigkernel.RBFKernel(sigma=self.rbf_sigma)
        else:
        ...
        score = signature_kernel.compute_scoring_rule(forecast, verification)
        return self.omega * score
