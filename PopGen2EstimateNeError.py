...
estimate_mcmc = np.zeros(shape=(len(p_err), len(s), len(r), n_rep, n_param))
MSE_mean_mcmc = np.zeros(shape=(len(p_err), len(s), len(r)))
MSE_std_mcmc = np.zeros(shape=(len(p_err), len(s), len(r)))
runtime_mcmc = np.zeros(shape=(len(p_err), len(s), len(r), n_rep))

estimate_lls = np.zeros(shape=(len(p_err), len(s), len(r), n_rep, n_param))
MSE_mean_lls = np.zeros(shape=(len(p_err), len(s), len(r)))
MSE_std_lls = np.zeros(shape=(len(p_err), len(s), len(r)))
runtime_lls = np.zeros(shape=(len(p_err), len(s), len(r), n_rep))

for ind_perr in range(len(p_err)):
        for ind_s in range(len(s)):
                for ind_r in range(len(r)):
                        print(ind_s, ind_r)
                        print('Running inference: true_s,', s[ind_s], 'true_r', r[ind_r], 'p_err', p_err[ind_perr])
                        ...
                        for ind_rep in range(n_rep):
                                ...
                                for ind_dim in range(data_obs_tmp_numpy.shape[1]-1):
                                        ...
                                        if p_t == 1.0:
                                                p_t= 1.0 - 1e-10
                                        if p_t == p_0:
                                                estimate_lls[ind_perr, ind_s, ind_r, ind_rep,ind_dim] = 0
                                        else:
                                                estimate_lls[ind_perr, ind_s, ind_r, ind_rep,ind_dim] = (np.log((p_t * (1-p_0))/(p_0*(1-p_t))))*(2/(10*nonzero_index))
                                        #print(p_t, p_0, estimate_lls[ind_s, ind_r, ind_rep,ind_dim])
                                end_t = time.time()
                                runtime_lls[ind_perr, ind_s, ind_r, ind_rep] = end_t - start_t
                        SE_lls = np.sqrt((1/WF.L) * np.sum(np.square(estimate_lls[ind_perr, ind_s, ind_r,:,:] - s[ind_s]), axis=1))
                        MSE_mean_lls[ind_perr, ind_s, ind_r], MSE_std_lls[ind_perr, ind_s, ind_r] = np.mean(SE_lls), np.std(SE_lls)
                        print(MSE_mean_lls, MSE_std_lls)
                        np.savez('Results/WF/selcof_2_lls_ne_error', estimate_lls=estimate_lls, MSE_mean_lls=MSE_mean_lls, MSE_std_lls=MSE_std_lls, runtime_lls=runtime_lls, data=data_obs)

                        def MCMC_temp_function(ind_rep):
                                data_obs_tmp = data_obs[:, ind_rep:ind_rep + 1, :]

                                from mysrc.ModelWF import WFUniform as WFU
                                pop_size = 5000 + (2*binom.rvs(1, .5)-1) * binom.rvs(5000, p_err[ind_perr])
                                WFU = WFU(neg_approx_llhd, population_size=pop_size, generation=100, generation_interval=[10],
                        ...
                        estimate_mcmc[ind_perr, ind_s, ind_r, :, :] = np.array(postmean)
                        runtime_mcmc[ind_perr, ind_s, ind_r, :] = np.array(r_time)

                        #print('estimate_mcmc', estimate_mcmc[ind_s, ind_r,:,:])
                        SE_mcmc = np.sqrt((1/WF.L) * np.sum(np.square(estimate_mcmc[ind_perr, ind_s, ind_r,:,:] - s[ind_s]), axis=1))
                        #print('SE', SE_mcmc)
                        MSE_mean_mcmc[ind_perr, ind_s, ind_r], MSE_std_mcmc[ind_perr, ind_s, ind_r] = np.mean(SE_mcmc), np.std(SE_mcmc)
                        print(MSE_mean_mcmc, MSE_std_mcmc)
                        np.savez('Results/WF/selcof_2_mcmc_ne_error', estimate_da=estimate_mcmc, MSE_mean=MSE_mean_mcmc, MSE_std=MSE_std_mcmc, runtime=runtime_mcmc, data = data_obs)